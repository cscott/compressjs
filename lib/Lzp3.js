...
var Window = function() {
  ...
  // initial context
  this.put(0xCA); this.put(0xFE); this.put(0xBA); this.put(0xBE);
};
...
Window.prototype.getIndex = function(s) {
  ...
  this.ctxt4[h4] = this.ctxt3[h3] = this.ctxt2[h2] = s+1;
...
LengthModel.prototype.encode = function(matchLen) {
  var lenlen = 0;
...
LengthModel.prototype.decode = function() {
...
Lzp3.compressFile = function(inStream, outStream, props) {
  ...
  HuffBit.prototype.encode = function(symbol) {
    ...
    if (this.table[this.root].weight > MAX24) {
      this.scale(1);
    }
  };

  var huffLiteral= new HuffBit((fileSize<0) ? 257 : 256);
  ...
  while (inSize !== fileSize) {
    ...
    if (ch===Util.EOF) {
      if (fileSize < 0) {
        huffLiteral.encode(256);
      }
      break;
    }
    huffLiteral.encode(ch);
    window.put(ch);
    inSize++;
  }
  flushBits();
