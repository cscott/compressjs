...
define(['./BitStream', './DefSumModel', './FenwickModel', './Huffman', './LogDistanceModel', './NoModel', './RangeCoder', './Stream', './Util'],function(BitStream, DefSumModel, FenwickModel, Huffman, LogDistanceModel, NoModel, RangeCoder, Stream, Util){
 ...
 */
Lzp3.compressFile = Util.compressFileHelper(LZP3_MAGIC, function(inStream, outStream, fileSize, props) {
  ...
  if (USE_HUFFMAN_CODE) {
    ...
    coderFactory = FenwickModel.factory(range, ESC_MAX_PROB);
    if (USE_DEFSUM) {
 ...
 */
Lzp3.decompressFile = Util.decompressFileHelper(LZP3_MAGIC, function(inStream, outStream, fileSize) {
