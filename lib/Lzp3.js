...
define(['./DeflateDistanceModel', './DefSumModel', './EscModel', './Huffman', './RangeCoder', './Util'],function(DeflateDistanceModel, DefSumModel, EscModel, Huffman, RangeCoder, Util){
...
var BitStream = function(stream) {
  this.readBit = (function() {
    ...
    return function() {
      if ((bufferByte & 0xFF) === 0) {
        bufferByte = (stream.readByte() << 1) | 1;
      }
  ...
  this.writeBit = (function() {
    var bufferByte = 1;
    return function(b) {
      bufferByte <<= 1;
      if (b) { bufferByte |= 1; }
      if (bufferByte & 0x100) {
        stream.writeByte(bufferByte & MAX8);
        bufferByte = 1;
      }
    };
  })();
  this.flush = function() {
    for (var i=0; i<7; i++) { this.writeBit(0); }
  };
};

var RescalingHuffman = function(bitstream, size) {
  Huffman.call(this, size, size, bitstream);
};
RescalingHuffman.factory = function(bitstream) {
  return function(size) { return new RescalingHuffman(bitstream, size); };
};
RescalingHuffman.prototype = Object.create(Huffman.prototype);
RescalingHuffman.prototype.encode = function(symbol) {
...
RescalingHuffman.prototype.decode = function() {
...
Lzp3.compressFile = function(inStream, outStream, props) {
  ...
  if (USE_HUFFMAN_CODE) {
    ...
    coderFactory = RescalingHuffman.factory(bitstream);
    ...
    coderFactory = EscModel.factory(range, MAX8);
    if (USE_DEFSUM) {
