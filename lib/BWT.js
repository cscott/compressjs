...
define(['./freeze'], function(freeze) {
    ...
    var LMSpostproc = function(T, SA, n, m) {
         ...
         * 2*m must not be larger than n (proveable) */
        ASSERT(n > 0);
    ...
    var SA_IS = function(T, SA, fs, n, k, isbwt) {
        ...
        if (k <= 256) {
            ...
            else { B = new Int32Array(k); flags = 3; }
        } else if (k <= fs) {
            C = SA.subarray(n + fs - k);
            if (k <= (fs - k)) { B = SA.subarray(n + fs - k * 2); flags = 0; }
            else if (k <= 1024) { B = new Int32Array(k); flags = 2; }
            else { B = C; flags = 8; }
        } else {
            C = B = new Int32Array(k);
            flags = 4 | 8;
        }
        ...
        if(name < m) {
            ...
            if((flags & 4) !== 0) { C = B = new Int32Array(k); }
            if((flags & 2) !== 0) { B = new Int32Array(k); }
        }
    ...
    /** Burrows-Wheeler Transform.
        ...
        alphabetSize is optional is T is an Uint8Array or Uint16Array.
    ...
    */
    BWT.bwtransform2 = function(T, U, n, alphabetSize) {
        ...
        if (T.length >= n*2) {
            ...
            TT = new Uint8Array(n*2);
        } else if (alphabetSize <= 65536) {
            TT = new Uint16Array(n*2);
        } else {
            TT = new Uint32Array(n*2);
        }
        if (TT!==T) {
