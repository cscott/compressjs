...
define(['./freeze','./makeBuffer','./BitStream','./BWT','./CRC32','./HuffmanAllocator','./Stream','./Util'], function(freeze, makeBuffer, BitStream, BWT, CRC32, HuffmanAllocator, Stream, Util) {
...
var StaticHuffman = function(outStream, freq, alphabetSize) {
  ...
  this.codeLengths = makeBuffer(alphabetSize);
  for (i=0; i<alphabetSize; i++) {
...
StaticHuffman.prototype.computeCanonical = function() {
  ...
  this.code = new Uint32Array(alphabetSize);
...
StaticHuffman.prototype.emit = function() {
  // write the starting length
  var i, currentLength = this.codeLengths[0];
  this.outStream.writeBits(5, currentLength);
  for (i=0; i<this.codeLengths.length; i++) {
    ...
    while (delta-- > 0) {
      this.outStream.writeBits(2, value);
    }
    this.outStream.writeBit(0);
    currentLength = codeLength;
  }
};
// encode the given symbol with this huffman code
StaticHuffman.prototype.encode = function(symbol) {
  this.outStream.writeBits(this.codeLengths[symbol], this.code[symbol]);
};
...
var compressBlock = function(block, length, outStream) {
  ...
  var piece = Math.floor(pos / (targetGroups-1)), cutoff = 0;
  for (i=0; i<pos; i++) {
    if (i >= cutoff && groups.length < (targetGroups-1)) {
      groups.push(new StaticHuffman(outStream, freq, endOfBlock+1));
      for (j=0; j<=endOfBlock; j++) { freq[j] = 0; }
      cutoff += piece;
    }
    freq[A[i]]++;
  }
  ...
  // okay, divide the block into groups at most GROUP_SIZE symbols long.
  var selectors = makeBuffer(Math.ceil(pos / GROUP_SIZE));
  // assign each group to the huffman table which compresses it best.
  for (i=0, k=0; i<pos; i+=GROUP_SIZE) {
    var groupSize = Math.min(GROUP_SIZE, pos - i);
    var best = 0, bestCost = groups[0].cost(A, i, groupSize);
    for (j=1; j<groups.length; j++) {
      var groupCost = groups[j].cost(A, i, groupSize);
  ...
  // XXX in theory we'd iterate here, by recomputing frequencies based on
  // which blocks were allocated to which selectors, and then redoing the
  // assignment of blocks to selectors.  But we're going to KISS.
...
Bzip2.compressFile = function(inStream, outStream, props) {
  ...
  if (blockSizeMultiplier < 1 || blockSizeMultiplier > 9) {
    throw new Error('Invalid block size multipler');
  }
  ...
  var block = makeBuffer(blockSize);
  var streamCRC = 0;
  var length;
