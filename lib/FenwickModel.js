...
var FenwickModel = function(coder, size, max_prob) {
    this.coder = coder;
    this.numSyms = size + 1; // save space for an escape symbol
    this.tree = new Uint32Array(this.numSyms*2);
    this.max_prob = (+max_prob) || DEFAULT_MAX_PROB;
    // sanity-check to prevent overflow.
    console.assert(this.max_prob <= 0xFFFF);
    console.assert(size <= 0xFFFF);
    // record escape probability as 1.
    var i;
    for (i=0; i<size; i++) {
        this.tree[this.numSyms + i] =
            (1 << ESC_SHIFT) | (0 << SYM_SHIFT);// escape prob=1, sym prob = 0
    }
    this.tree[this.numSyms + i] =
        (0 << ESC_SHIFT) | (1 << SYM_SHIFT); // escape prob = 0, sym prob = 1
    this._sumTree();
    // probability sums are in this.tree[1].  this.tree[0] is unused.
};
FenwickModel.factory = function(coder, max_prob) {
    return function(size) { return new FenwickModel(coder, size, max_prob); };
};
FenwickModel.prototype.encode = function(symbol) {
    var i = this.numSyms + symbol;
    var sy_f = this.tree[i];
    var mask = SYM_MASK, update = (1 << SYM_SHIFT), shift = SYM_SHIFT;
...
FenwickModel.prototype._rescale = function() {
    ...
    for (i=0; i < this.numSyms-1; i++) {
        var prob = this.tree[this.numSyms + i];
        if ((prob & ESC_MASK) !== 0) {
            // this symbol escapes
            noEscape = false;
            continue;
...
FenwickModel.compressFile = Util.compressFileHelper(FENWICK_MAGIC, function(inStream, outStream, fileSize, props, finalByte) {
...
FenwickModel.decompressFile = Util.decompressFileHelper(FENWICK_MAGIC, function(inStream, outStream, fileSize) {
