 ...
 */
Lzjb.compressFile = Util.compressFileHelper(LZJB_MAGIC, function(inStream, outStream, fileSize, props, finalByte) {
    ...
    var MATCH = 256;
    var EOF_SYM = 257;
    var literalModel = new MTFModel(encoder,
                                    ((fileSize<0) ? EOF_SYM : MATCH) + 1);
    var lenModel = new MTFModel(encoder, (MATCH_MAX-MATCH_MIN)+1);
    var posModel = new MTFModel(encoder, OFFSET_MASK+1);
    while (true) {
 ...
 */
Lzjb.decompressFile = Util.decompressFileHelper(LZJB_MAGIC, function(inStream, outStream, outSize) {
    var sstart, dstart = [], slen,
        src = 0, dst = 0,
        cpy, copymap,
        mlen, offset,
    ...
    var MATCH = 256;
    var EOF_SYM = 257;
    var literalModel = new MTFModel(decoder,
                                    ((outSize<0) ? EOF_SYM : MATCH) + 1);
    var lenModel = new MTFModel(decoder, (MATCH_MAX-MATCH_MIN)+1);
    var posModel = new MTFModel(decoder, OFFSET_MASK+1);
