...
Lzjb.compressFile = function(inStream, outStream, props) {
    ...
    var MATCH = 256;
    var EOF_SYM = 257;
    var literalModel = new EscModel(encoder,
                                    ((fileSize<0) ? EOF_SYM : MATCH) + 1);
    var lenModel = new EscModel(encoder, (MATCH_MAX-MATCH_MIN)+1);
    var posModel = new EscModel(encoder, OFFSET_MASK+1);
    while (true) {
...
Lzjb.decompressFile = function(inStream, outStream) {
    ...
    var MATCH = 256;
    var EOF_SYM = 257;
    var literalModel = new EscModel(decoder,
                                    ((outSize<0) ? EOF_SYM : MATCH) + 1);
    var lenModel = new EscModel(decoder, (MATCH_MAX-MATCH_MIN)+1);
    var posModel = new EscModel(decoder, OFFSET_MASK+1);
