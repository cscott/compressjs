 ...
 * ps. some truely grotty C code in the originally, faithfully ported to
...
//  of nodes to be used
var Huff = function(size, root, bitstream) {
  var i;
...
Huff.prototype.split = function(symbol) {
  var pair, node;
...
Huff.prototype.leader = function(node) {
...
Huff.prototype.slide = function(node) {
  var next = node;
  var swap;
...
Huff.prototype.increment = function(node) {
  var up;
...
Huff.prototype.scale = function(bits) {
...
Huff.prototype.sendid = function(symbol) {
  var empty = 0, max;
...
Huff.prototype.encode = function(symbol) {
  var emit = 1, bit;
  var up, idx, node;
...
Huff.prototype.readid = function() {
...
Huff.prototype.decode = function() {
  var node = this.root;
  var symbol, down;
...
var RESCALE_MASK = 8191;

// standalone compressor, mostly for testing
Huff.compressFile = function(input, output) {
  ...
  Util.writeUnsignedNumber(output, size+1);

  var alphabetSize = 256;
  if (size < 0) { alphabetSize++; }
  var huff = new Huff(257, alphabetSize);
  var bufferByte = 1;
  huff.writeBit = function(b) {
    bufferByte <<= 1;
    if (b) { bufferByte |= 1; }
    if (bufferByte & 0x100) {
      output.writeByte(bufferByte & 0xFF);
      bufferByte = 1;
    }
  };
  huff.flushBits = function() {
    while (bufferByte !== 1) {
      huff.writeBit(0);
    }
  };
  ...
  while (i !== size) {
    ...
    i++;
    if ((i & RESCALE_MASK) === 0) {
      // XXX is this sufficient?  might still overflow, i think [csa]
      huff.scale(1);
    }
  }
  if (size < 0) {
    huff.encode(alphabetSize-1); // EOF marker
  }
  huff.flushBits();
  return o.retval;
};

// standalone decompressor, again for testing
Huff.decompressFile = function(input, output) {
  input = Util.coerceInputStream(input);
  var size = Util.readUnsignedNumber(input) - 1;
  var o = Util.coerceOutputStream(output, size);
  output = o.stream;
  ...
  huff.readBit = function() {
    if ((bufferByte & 0xFF) === 0) {
      bufferByte = (input.readByte() << 1) | 1;
    }
