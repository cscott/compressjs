...
var DefSumModel = function(coder, size, isDecoder) {
  ...
  this.coder = coder;
  this.prob = new Uint16Array(size+2); /* size + ESC + 1 */
  this.escape = new Uint16Array(size+1);  /* size + 1*/
  this.update = new Uint16Array(size+1); /* size + ESC */
  ...
  this.probToSym = new Uint16Array(PROB_TOTAL);
  this.escProbToSym = new Uint16Array(this.numSyms);
...
var DEFSUM_MAGIC='dfsm';
DefSumModel.compressFile = Util.compressFileHelper(DEFSUM_MAGIC, function(inStream, outStream, fileSize, props, finalByte) {
...
DefSumModel.decompressFile = Util.decompressFileHelper(DEFSUM_MAGIC, function(inStream, outStream, fileSize) {
